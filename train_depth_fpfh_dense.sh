...
GPU_ID=${1:-0}
EXPERIMENT=${2:-"Depth_fpfh_dense"}
FPFH_RADIUS=${3:-1.0}       # Dense FPFH용 radius
GT_RADIUS=${4:-3}            # GT matching radius
CONF="gluefactory/configs/3Dlightglue_fpfh.yaml"

# 1) Dense FPFH precompute (캐시가 없을 때만 실행)
CACHE_DIR="gluefactory/datasets/mitsubishi/fpfh_cache_r${FPFH_RADIUS}"
if [ ! -d "$CACHE_DIR" ]; then
    echo "=== Precompute Dense FPFH (radius=${FPFH_RADIUS}) ==="
    python3 precompute_fpfh_dense.py --fpfh_radius "$FPFH_RADIUS"
else
