...
def build_xyz_pcd(depth_crop_raw, fx, fy, cx, cy, mask=None):
    """depth crop → 진짜 (X,Y,Z) point cloud 생성 (FPFH 계산용, 단위: mm).

    ISS 검출은 (u,v,depth_scaled) 공간에서 하되,
    FPFH는 view-invariant하려면 진짜 3D 좌표가 필요하므로 별도 PCD 생성.

    Args:
        mask: erosion 마스크. None이면 depth>0 영역 전체 사용.
    Returns:
        pcd: Open3D PointCloud (X, Y, Z in mm)
    """
    if mask is None:
        mask = (depth_crop_raw > 0).astype(np.uint8)
    ...
    X = (us.astype(np.float64) - cx) * depth_real / fx
    Y = (vs.astype(np.float64) - cy) * depth_real / fy
    Z = depth_real
...
def kp_crop_to_xyz(kp_crop, n_valid, depth_crop_raw, fx, fy, cx, cy):
    """ISS keypoint의 crop 좌표 (u,v) → 진짜 (X,Y,Z) 변환 (FPFH KDTree 조회용).

    Args:
        kp_crop: (max_num, 2) crop 좌표 (u,v)
        n_valid: 유효 keypoint 수
    Returns:
        kp_xyz: (max_num, 3) 진짜 3D 좌표 (X,Y,Z in mm)
    """
...
def compute_fpfh_for_keypoints(pcd, kp_3d, n_valid,
    ...
    # KDTree로 keypoint 위치에서 lookup (3D 좌표 직접 사용)
    kdtree = o3d.geometry.KDTreeFlann(pcd)
    for i in range(n_valid):
        query = kp_3d[i]  # (u, v, depth_real) — 이미 3D 좌표
...
def main():
    ...
    # 카메라 intrinsics (5761×5761 전체 이미지 기준, crop offset은 자동 보정)
    ...
    parser.add_argument("--cx_orig", type=float, default=2880.5,
                        help="principal point x (원본 5761px 기준)")
    parser.add_argument("--cy_orig", type=float, default=2880.5,
                        help="principal point y (원본 5761px 기준)")
    parser.add_argument("--force", action="store_true")
    args = parser.parse_args()

    # crop 좌표계 기준 principal point 보정
    cx = args.cx_orig - CROP_X0  # 2880.5 - 1129 = 1751.5
    cy = args.cy_orig - CROP_Y0  # 2880.5 - 1081 = 1799.5

    base_dir = Path("gluefactory/datasets/mitsubishi")
    img_dir = base_dir / "dataset_resample_2"
    cache_dir = base_dir / f"iss_fpfh_resample2_cache_r{args.fpfh_radius}_xyz"
    ...
    print(f"Config: max_kp={args.max_num_keypoints}, gamma_21={args.gamma_21}, "
          f"gamma_32={args.gamma_32}, fpfh_radius={args.fpfh_radius}mm, "
          f"image_size={args.image_size}")
    print(f"Intrinsics (crop): fx={args.fx}, fy={args.fy}, cx={cx:.1f}, cy={cy:.1f}")
    print(f"Crop: ({CROP_X0}, {CROP_Y0}), size={CROP_SIZE}")

    stats = {"total": 0, "iss_counts": [], "valid_counts": []}

    for img_path in tqdm(image_paths, desc="Precomputing ISS+FPFH (resample_2, xyz)"):
        ...
        # FPFH용 진짜 3D PCD (카메라 intrinsics 사용)
        pcd_xyz = build_xyz_pcd(img_crop, args.fx, args.fy, cx, cy, mask=erode_mask)

        # ISS keypoint crop 좌표 → 진짜 (X,Y,Z) 변환 (FPFH lookup용)
        kp_xyz = kp_crop_to_xyz(kp_crop, n_valid, img_crop, args.fx, args.fy, cx, cy)

        # FPFH 계산 (진짜 3D PCD 기반 → view-invariant)
    ...
    json.dump({**vars(args), "cx_crop": cx, "cy_crop": cy}, open(config_path, "w"), indent=2)
