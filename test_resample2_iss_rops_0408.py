"""
학습된 ISS(det)+RoPS(desc)+LG 모델로 resample_2 데이터 매칭 테스트.

사용법:
    python test_resample2_iss_rops_0408.py --indices 0 10 50 90
    python test_resample2_iss_rops_0408.py --experiment 0408_resample2_iss_rops135_lg --split test --num_samples 10
"""
...
def main():
    parser = argparse.ArgumentParser(description="학습된 ISS+RoPS+LG로 resample_2 매칭 테스트")
    parser.add_argument("--checkpoint", type=str, default=None)
    parser.add_argument("--experiment", type=str, default="0408_resample2_iss_rops135_lg")
    ...
    print(f"{args.split} dataset: {len(dataset)} pairs "
          f"(crop={CROP_SIZE}, image_size={image_size})")
    ...
    for idx in indices:
        sample = dataset[idx]
        batch = resample2_iss_rops_collate_fn([sample])
