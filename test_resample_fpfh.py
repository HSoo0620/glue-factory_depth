"""
학습된 SP+FPFH+LG 모델로 resample 데이터 매칭 테스트.
...
def visualize_pair(pred, data, idx, output_path, csv_path=None, gt_radius=3,
    ...
    info = f"[SP+FPFH+LG]  Keypoints: {kp0.shape[0]} / {kp1.shape[0]}  |  Total matches: {n_total}"
...
def main():
    ...
    for idx in indices:
        ...
        visualize_pair(pred, batch, 0, output_path, csv_path=str(csv_path),
