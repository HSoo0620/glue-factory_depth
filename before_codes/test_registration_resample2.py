"""
학습된 SP+LG 모델로 resample_2 데이터 3D Registration 테스트.

resample_2는 물리적으로 보정된 이미지이므로 ini(카메라 파라미터) 없이
(u, v, depth_real)을 직접 3D 좌표로 사용.

파이프라인:
    1. 학습된 SP+LG로 crop+resize된 depth image pair 매칭
    2. 매칭된 keypoint를 원본 5761 좌표로 복원 → (u, v, depth_real)로 3D 좌표 생성
    3. RANSAC + SVD로 rigid transformation (R, t) 추정
    4. 변환 적용 후 정합 결과 시각화

사용법:
    python test_registration_resample2.py --experiment 0328_resample2_sp_lg --indices 0 10 50 90
    python test_registration_resample2.py --checkpoint outputs/training/0328_resample2_sp_lg/checkpoint_best.tar --conf_th 0.1 --inlier_th 5.0
"""
...
from gluefactory.datasets.mitsubishi_resample2_dataset import (
    MitsubishiResample2Dataset,
    resample2_collate_fn,
    CROP_X0,
    CROP_Y0,
    CROP_SIZE,
)
...
CLIP_END = 1000.0
ORIG_SIZE = 5761
GRID_DX = 0.05  # 픽셀→물리 변환 (모든 이미지 동일)
GRID_DY = 0.05


# ─── 2D+Depth → 3D (ini 없이, u,v,depth 직접 사용) ──────────

def pixel_to_3d_direct(keypoints_2d, depth_map_raw):
    """2D keypoint + raw depth → 3D 좌표 (u, v, depth_real).

    ini를 사용하지 않고 (u, v, depth_real)을 직접 3D 좌표로 사용.
    resample_2 데이터는 물리적으로 보정되어 있어 변환 불필요.
...
def kpts_to_orig(kpts, image_size):
    ...
    kpts_crop = kpts * scale  # crop 좌표계로
    kpts_orig = kpts_crop.copy()
    kpts_orig[:, 0] += CROP_X0  # 원본 좌표계로
    kpts_orig[:, 1] += CROP_Y0
    return kpts_orig
...
def warp_depth_to_master(depth1_raw, R_est, t_est):
    ...
    # 3D 좌표: (u*grid_dx, v*grid_dy, depth_real) — 물리 단위 통일
...
def visualize_registration(
    ...
    # (u, v, depth_real) 직접 3D 좌표
    pts0, vmask0 = pixel_to_3d_direct(mkp0_orig, depth0_raw)
    pts1, vmask1 = pixel_to_3d_direct(mkp1_orig, depth1_raw)
    ...
    R_est, t_est, inliers = ransac_rigid(pts1_valid, pts0_valid, inlier_th=inlier_th)
    ...
    fig.suptitle(
        f"[Resample2 SP+LG]  Matches: {n_matches}  |  3D: {n_3d}  |  "
...
def main():
    parser = argparse.ArgumentParser(description="학습된 SP+LG 기반 resample_2 3D Registration")
    parser.add_argument("--checkpoint", type=str, default=None)
    parser.add_argument("--experiment", type=str, default="0328_resample2_sp_lg")
    ...
    # combination.csv에서 원본 이미지 경로를 가져오기 위해 직접 로드
    base_dir = Path("gluefactory/datasets/mitsubishi")
    combo = pd.read_csv(base_dir / "outputs_resample_2" / "combination.csv")
    total = len(combo)
    ...
    dataset = MitsubishiResample2Dataset(split=args.split, image_size=image_size)
    print(f"{args.split} dataset: {len(dataset)} pairs (crop={CROP_SIZE}, image_size={image_size})")
    ...
    for idx in indices:
        ...
        # 모델 추론
        sample = dataset[idx]
        batch = resample2_collate_fn([sample])
