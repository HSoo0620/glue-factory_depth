...
사용법:
    python precompute_fpfh_dense.py
    python precompute_fpfh_dense.py --max_num_keypoints 512 --detection_threshold 0.003
    python precompute_fpfh_dense.py --fpfh_radius 1.0 --fpfh_max_nn 100
    python precompute_fpfh_dense.py --force  # 기존 캐시 덮어쓰기
...
def compute_fpfh_for_keypoints(depth_map_real, keypoints, n_valid, K,
    ...
    """
    Dense point cloud 전체에서 FPFH를 계산한 뒤,
    각 keypoint 위치에서 가장 가까운 점의 descriptor를 lookup.

    Args:
        depth_map_real: 실제 depth 값 (clip 적용된 단위)
    ...
    fpfh_dense = np.array(fpfh.data).T.astype(np.float32)  # (N_dense, 33)
...
def main():
    parser = argparse.ArgumentParser(description="SuperPoint keypoints + Dense FPFH precomputation")
    ...
    print(f"Mode: Dense FPFH -> keypoint lookup")

    stats = {"total": 0, "valid_counts": []}

    for img_path in tqdm(image_paths, desc="Precomputing Dense FPFH"):
