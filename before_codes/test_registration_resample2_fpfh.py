"""
학습된 SP(det)+FPFH(desc)+LG 모델로 resample_2 데이터 3D Registration 테스트.

resample_2는 물리적으로 보정된 이미지이므로 ini(카메라 파라미터) 없이
(u, v, depth_real)을 직접 3D 좌표로 사용.

파이프라인:
    1. 학습된 SP+FPFH+LG로 crop+resize된 depth image pair 매칭
    2. 매칭된 keypoint를 원본 5761 좌표로 복원 → (u, v, depth_real)로 3D 좌표 생성
    3. RANSAC + SVD로 rigid transformation (R, t) 추정
    4. 변환 적용 후 정합 결과 시각화

사용법:
    python test_registration_resample2_fpfh.py --experiment 0331_resample2_sp_fpfh_lg --indices 0 10 50 90
    python test_registration_resample2_fpfh.py --checkpoint outputs/training/0331_resample2_sp_fpfh_lg/checkpoint_best.tar --conf_th 0.1 --inlier_th 5.0
"""
...
from gluefactory.datasets.mitsubishi_resample2_fpfh_dataset import (
    MitsubishiResample2FPFHDataset,
    resample2_fpfh_collate_fn,
    CROP_X0,
    CROP_Y0,
    CROP_SIZE,
)
...
def visualize_registration(
    ...
    fig.suptitle(
        f"[Resample2 SP+FPFH+LG]  Matches: {n_matches}  |  3D: {n_3d}  |  "
...
def main():
    parser = argparse.ArgumentParser(description="학습된 SP+FPFH+LG 기반 resample_2 3D Registration")
    parser.add_argument("--checkpoint", type=str, default=None)
    parser.add_argument("--experiment", type=str, default="0331_resample2_sp_fpfh_lg")
    ...
    dataset = MitsubishiResample2FPFHDataset(
    ...
    for idx in indices:
        ...
        # 모델 추론
        sample = dataset[idx]
        batch = resample2_fpfh_collate_fn([sample])
