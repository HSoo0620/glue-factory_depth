"""
학습된 SP+Hybrid(SP+FPFH)+LG 모델로 resample 데이터 3D Registration 테스트.

파이프라인:
    1. Hybrid 캐시에서 289D descriptor 로드
    2. 학습된 Hybrid+LG로 매칭
...
사용법:
    python test_registration_hybrid.py --experiment 0324_resample_sp_hybrid_lg --fpfh_radius 0.5 --indices 0 10 50 90
    python test_registration_hybrid.py --conf_th 0.1 --inlier_th 5.0
"""
...
from gluefactory.datasets.mitsubishi_resample_hybrid_dataset import (
    MitsubishiResampleHybridDataset,
    resample_hybrid_collate_fn,
)
...
def visualize_registration(
    ...
    fig.suptitle(
        f"[SP+Hybrid+LG]  Matches: {n_matches}  |  3D: {n_3d}  |  "
...
def main():
    parser = argparse.ArgumentParser(description="SP+Hybrid+LG 기반 resample 3D Registration")
    ...
    dataset = MitsubishiResampleHybridDataset(
    ...
    for idx in indices:
        ...
        orig_size = depth0_raw.shape[0]

        sample = dataset[idx]
        batch = resample_hybrid_collate_fn([sample])
