"""
Rule-based feature matching Registration 평가 — SIFT / ORB + FLANN.

학습 기반(ISS+SHOT+LG, ISS+FPFH+LG 등)과의 비교용 baseline.
eval_registration_iss_shot.py와 동일한 RMSE 평가 파이프라인 사용.

좌표계: (u*grid_dx, v*grid_dy, depth_real) — RANSAC, R,t 추정, RMSE 모두 동일 공간

사용법:
    python eval_registration_rule_based.py --method sift --indices 0 10 50 90
    python eval_registration_rule_based.py --method orb --indices 0 10 50 90
"""
...
# ─── Core math (eval_registration_iss_shot.py 동일) ───────
...
def depth_to_uint8(depth_raw):
    """Depth uint16 -> uint8 (유효 영역만 정규화, depth=0은 0 유지)."""
    mask = depth_raw > 0
    if not mask.any():
    ...
    normalized = (depth_raw.astype(np.float64) - d_min) / (d_max - d_min + 1e-8)
...
def create_flann_matcher(method):
    """FLANN matcher 생성 (SIFT: KDTree, ORB: LSH)."""
    if method == "sift":
...
def detect_and_match(img0_u8, img1_u8, detector, method, ratio_th=0.75):
    """Feature detection + FLANN matching + Lowe's ratio test.

    Returns:
        mkp0, mkp1: (M, 2) matched keypoints (crop 좌표)
        n_kp0, n_kp1: 검출된 전체 keypoint 수
    """
...
def crop_to_orig(kp_crop):
    """Crop 좌표 → 원본(5761) 좌표."""
    kp_orig = kp_crop.copy()
    kp_orig[:, 0] += CROP_X0
    kp_orig[:, 1] += CROP_Y0
    return kp_orig
...
def visualize_registration(depth0_raw, depth1_raw, R_est, t_est,
                           output_path, method, n_matches, n_3d,
    ...
    fig.suptitle(
        f"[{method.upper()}+FLANN]  Matches: {n_matches}  |  3D: {n_3d}  |  "
...
def evaluate_one_direction(mkp_src_orig, mkp_dst_orig,
    ...
    Returns:
        dict with rmse, n_matches, n_3d, n_inliers, R_est, t_est
        or None if failed.
    """
    ...
    # RANSAC: src -> dst (input -> master for forward)
...
def evaluate_pair(depth0_raw, depth1_raw, gt_csv_path, detector, method, args):
    """단일 pair 양방향 평가."""
    ...
    mkp0_crop, mkp1_crop, n_kp0, n_kp1 = detect_and_match(
        img0_u8, img1_u8, detector, method, ratio_th=args.ratio_th)
    print(f"  Keypoints: view0={n_kp0}, view1={n_kp1}")

    fwd = evaluate_one_direction(
        crop_to_orig(mkp1_crop), crop_to_orig(mkp0_crop),  # input -> master
    ...
    mkp1r_crop, mkp0r_crop, _, _ = detect_and_match(
        img1_u8, img0_u8, detector, method, ratio_th=args.ratio_th)

    R_gt_rev = R_gt.T
    t_gt_rev = -R_gt.T @ t_gt

    rev = evaluate_one_direction(
        crop_to_orig(mkp0r_crop), crop_to_orig(mkp1r_crop),  # master -> input
...
def main():
    parser = argparse.ArgumentParser(
        description="Rule-based (SIFT/ORB + FLANN) Registration Eval")
    ...
    output_dir = Path(
        ...
        else f"results/registration/{args.method}_flann_eval")
    ...
    results = []
    for idx in tqdm(indices, desc=f"{args.method.upper()}+FLANN"):
        print(f"\n--- Pair {idx} ---")
        ...
        res = evaluate_pair(
            depth0_raw, depth1_raw, csv_path, detector, args.method, args)
