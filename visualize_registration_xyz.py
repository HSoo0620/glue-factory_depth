"""
Registration 시각화 — (X, Y, Z) mm 좌표계 버전.

카메라 intrinsics 기반 실제 3D 좌표로 포인트 클라우드 정합 시각화.
RMSE도 순수 mm 단위.

사용법:
    # SHOT
    python visualize_registration_xyz.py --experiment 0407_resample2_iss_shot352_lg --descriptor shot --indices 0 10 50
    # RoPS
    python visualize_registration_xyz.py --experiment 0408_resample2_iss_rops135_lg --descriptor rops --indices 0 10 50
    # FPFH
    python visualize_registration_xyz.py --experiment 0406_resample2_iss_fpfh_xyz_lg --descriptor fpfh --indices 0 10 50
"""
...
def compute_gt_transform(gt_csv_path, depth0_raw, depth1_raw):
    ...
    for i in range(len(master_xy)):
        ...
        pts_m.append([(mu - CX_ORIG) * Z0 / FX, (mv - CY_ORIG) * Z0 / FY, Z0])
        pts_i.append([(iu - CX_ORIG) * Z1 / FX, (iv - CY_ORIG) * Z1 / FY, Z1])
...
def plot_alignment(pc_master, pc_input, pc_est, pc_gt, title, output_path,
    ...
    """3-row × 3-col: (X-Y), (X-Z), (Y-Z) projections."""
    fig, axes = plt.subplots(3, 3, figsize=(18, 16))
    s = 0.5
    alpha = 0.6

    labels_row = ["Before", "Estimated R,t", "GT R,t"]
    labels_col = ["Top (X, Y) mm", "Front (X, Z) mm", "Side (Y, Z) mm"]
    ...
    for row in range(3):
        axes[row, 0].invert_yaxis()  # top: Y 아래로 증가 (이미지 좌표 관례)
        axes[row, 1].invert_yaxis()  # front: Z 아래로 증가
        axes[row, 2].invert_yaxis()  # side: Z 아래로 증가
...
def plot_overlay_detail(pc_master, pc_est, title, output_path):
    """Estimated alignment 확대."""
    fig, axes = plt.subplots(1, 3, figsize=(18, 5))
    s = 0.5
    alpha = 0.6
    col_axes = [(0, 1), (0, 2), (1, 2)]
    col_labels = ["Top (X, Y) mm", "Front (X, Z) mm", "Side (Y, Z) mm"]
...
def main():
    parser = argparse.ArgumentParser(
        description="Registration visualization — XYZ mm coordinates")
    ...
    output_dir = Path(
        ...
        else f"results/registration/{args.experiment}_vis_xyz")
    ...
    for idx in indices:
        ...
        pc_master = sample_point_cloud_xyz(depth0_raw, args.n_sample_pts)
        pc_input  = sample_point_cloud_xyz(depth1_raw, args.n_sample_pts)

        pc_est = (R_est @ pc_input.T).T + t_est
        pc_gt  = (R_gt  @ pc_input.T).T + t_gt

        P_30k = sample_point_cloud_xyz(depth1_raw, 30000)
