#!/bin/bash
# ISS(detector) + FPFH(descriptor, Grid좌표) + LG 학습 (resample_2 데이터)
# ISS+FPFH 캐시 없으면 자동 생성 (Grid 좌표계: u*grid_dx, v*grid_dy, depth_real)

GPU_ID=${1:-3}
EXPERIMENT=${2:-"0410_resample2_iss_fpfh_grid_lg"}
FPFH_RADIUS=${3:-10.0}
IMAGE_SIZE=${4:-1751}
GT_RADIUS=${5:-20}
BATCH_SIZE=${6:-32}
RESTORE=${7:-""}
CONF="gluefactory/configs/0410_resample2_iss_fpfh_lg.yaml"
...
# ISS+FPFH 캐시 확인 및 생성 (Grid 좌표계)
CACHE_DIR="gluefactory/datasets/mitsubishi/iss_fpfh_resample2_cache_r${FPFH_RADIUS}_grid"
if [ ! -d "$CACHE_DIR" ] || [ -z "$(ls -A $CACHE_DIR/*.npz 2>/dev/null)" ]; then
    echo "=== ISS+FPFH (Grid) cache not found. Precomputing... ==="
    python3 precompute_iss_fpfh_resample2_grid.py \
