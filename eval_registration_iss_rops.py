"""
ISS+RoPS+LG Registration 평가 — Transform 기반 RMSE.
...
사용법:
    python eval_registration_iss_rops.py --experiment 0408_resample2_iss_rops135_lg --indices 0 10 50 90
"""
...
from gluefactory.datasets.mitsubishi_resample2_iss_rops_dataset import (
    MitsubishiResample2ISSRoPSDataset,
    resample2_iss_rops_collate_fn,
    CROP_X0,
    CROP_Y0,
    CROP_SIZE,
)
...
def ransac_rigid(src, dst, n_iter=1000, inlier_th=5.0):
    ...
    Returns: R (3,3), t (3,), inlier_mask (N, bool)
    """
    N = src.shape[0]
    if N < 3:
...
def extract_matches(pred, batch_idx, conf_th=0.0):
    """모델 예측에서 매칭 pair 추출. Returns: mkp0, mkp1, n_matches."""
...
def save_overlay(depth0_raw, depth1_raw, R_est, t_est, output_path):
    ...
    depth_real = CLIP_START + (d_raw / 65535.0) * (CLIP_END - CLIP_START)
    pts = np.stack([us * GRID_DX, vs * GRID_DY, depth_real], axis=1)
...
def evaluate_pair(model, dataset, collate_fn, idx, combo_row,
    ...
    print(f"  Forward: matches={n_match_fwd}, 3D={n_3d_fwd}, "
          f"inliers={n_inl_fwd}")
    print(f"  err: {err_before:.3f} -> {err_after:.3f}")
    ...
    print(f"  Transform RMSE(fwd): {rmse_fwd:.4f}")
    ...
    pts0_rev, vm0r = pixel_to_grid3d(mkp0_rev_orig, depth1_raw)
    pts1_rev, vm1r = pixel_to_grid3d(mkp1_rev_orig, depth0_raw)
    both_r = vm0r & vm1r
...
def main():
    parser = argparse.ArgumentParser(
        description="ISS+RoPS+LG Registration Eval (Transform RMSE)")
    ...
    dataset = MitsubishiResample2ISSRoPSDataset(
        split=args.split, image_size=image_size)
