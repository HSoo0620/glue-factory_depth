"""
Resample_2 depth 이미지에 대해 ISS keypoints + C++ RoPS135 descriptor 계산.
precompute_iss_shot352_resample2.py 와 동일한 구조, SHOT352→RoPS135 교체.

RoPS135: mesh-based 3D descriptor, 135D, L1-normalized (C++ 출력).
         → L2 normalize 후 캐시 저장.
...
사용법:
    ...
    python precompute_iss_rops_resample2.py                    # 전체
    python precompute_iss_rops_resample2.py --max_images 3 --force  # 스모크 테스트

출력:
    gluefactory/datasets/mitsubishi/iss_rops135_resample2_cache/{stem}.npz
    npz: keypoints(512,2), keypoint_scores(512,), rops_descriptors(512,135),
...
def load_rops135_bin(bin_path):
    """C++ RoPS135 bin 로드.
    Returns:
        pts:  (N, 3) float32 XYZ in mm (full original image 기준)
        desc: (N, 135) float32, L2 normalized
    """
    with open(bin_path, 'rb') as f:
        N, D = np.frombuffer(f.read(8), dtype=np.uint32)
        N, D = int(N), int(D)
        assert D == ROPS_DIM, f"Expected D={ROPS_DIM}, got {D}"
        data = np.frombuffer(f.read(N * (3 + D) * 4), dtype=np.float32)
    data = data.reshape(N, 3 + D)
    pts  = data[:, :3]
    desc = data[:, 3:]
    # zero descriptor 및 NaN 필터링
    valid = np.any(desc != 0, axis=1) & ~np.isnan(desc).any(axis=1)
    pts, desc = pts[valid], desc[valid]
    # L1-normalized → L2 normalize
...
def lookup_rops135(pts_cloud, desc_cloud, kp_xyz, n_valid):
    """ISS keypoint XYZ → KDTree lookup in C++ cloud → RoPS135 descriptors.
    Returns:
        rops_out: (max_num, 135) float32
    """
    max_n = kp_xyz.shape[0]
    rops_out = np.zeros((max_n, ROPS_DIM), dtype=np.float32)
    if n_valid < 1 or len(pts_cloud) < 1:
        return rops_out
    tree = cKDTree(pts_cloud)
    _, idxs = tree.query(kp_xyz[:n_valid])
    rops_out[:n_valid] = desc_cloud[idxs]
    return rops_out


# -- ISS 함수 (precompute_iss_shot352_resample2.py 와 동일) ──────────────────
...
def main():
    ...
    parser.add_argument("--bin_dir", type=str,
                        default="gluefactory/datasets/Descriptor/output_rops_10_omp")
    ...
    cache_dir = base_dir / "iss_rops135_resample2_cache"
    ...
    print(f"Config: RoPS135 (135D, L2-norm), C++ bins from {bin_dir.name}/")
    print(f"Cache -> {cache_dir}/")

    stats = {"iss_counts": [], "valid_counts": []}

    for img_path in tqdm(image_paths, desc="Precomputing ISS+RoPS135"):
        out_path = cache_dir / f"{img_path.stem}.npz"
        if out_path.exists() and not args.force:
            continue

        # -- C++ RoPS135 bin 로드 --
        bin_path = bin_dir / f"{img_path.stem}_rops135.bin"
        if not bin_path.exists():
            print(f"\n  [SKIP] bin not found: {bin_path.name}")
            continue
        pts_cloud, desc_cloud = load_rops135_bin(bin_path)
        ...
        # -- C++ cloud KDTree lookup → RoPS135 --
        rops = lookup_rops135(pts_cloud, desc_cloud, kp_xyz, n_valid)

        np.savez_compressed(
            out_path,
            keypoints         = keypoints,
            keypoint_scores   = scores,
            rops_descriptors  = rops,      # (512, 135)
