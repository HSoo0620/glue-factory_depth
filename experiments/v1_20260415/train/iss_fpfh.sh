...
#   기존 체크포인트(iss_fpfh_v1_20260415)는 padding GT row 가 valid 로 통과되어 오염된 그래디언트로 학습됨.
# 실행: bash experiments/v1_20260415/train/iss_fpfh.sh  (repo root 기준)
set -e
...
GPU=${GPU:-0}
EXP=${EXP:-iss_fpfh_v1_20260415_0417}
BATCH=${BATCH:-4}
RESTORE=${RESTORE:-}
...
if [ ! -d "$CACHE_DIR" ] || [ -z "$(ls -A $CACHE_DIR 2>/dev/null)" ]; then
    echo "[v1] FPFH cache missing -> run precompute first."
    exit 1
fi
