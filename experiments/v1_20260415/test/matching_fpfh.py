"""v1 (2026-04-15) ISS+FPFH+LG 매칭 시각화.

학습된 ISS(det)+FPFH(desc)+LG 모델로 v1_20260415 테스트 페어를 매칭하고 4색 시각화.
...
Crop/Resize 없음. FPFH descriptor는 raw 33D (L2-normalize 미적용).

사용법:
    python experiments/v1_20260415/test/matching_fpfh.py --experiment iss_fpfh_v1_20260415
    python experiments/v1_20260415/test/matching_fpfh.py --experiment iss_fpfh_v1_20260415 --indices 0 10 50 90
    python experiments/v1_20260415/test/matching_fpfh.py \
        --weights outputs/training/iss_fpfh_v1_20260415/checkpoint_best.tar
"""
...
from gluefactory.datasets.mitsubishi_v1_20260415_iss_fpfh_dataset import (
    MitsubishiV1ISSFPFHDataset,
    v1_iss_fpfh_collate_fn,
    PAD_H,
    PAD_W,
)
...
DEFAULT_WEIGHTS = "outputs/training/iss_fpfh_v1_20260415/checkpoint_best.tar"
DEFAULT_OUTPUT_DIR = "results/iss_fpfh_v1_20260415"
DEFAULT_CONFIG = "gluefactory/configs/iss_fpfh_v1_20260415_lg.yaml"
...
def load_model(checkpoint_path, device, config_path=DEFAULT_CONFIG):
    ...
    if "conf" in cp:
        ...
        # fallback: load training config from repo (shouldn't typically be needed)
...
def main():
    parser = argparse.ArgumentParser(
        description="v1_20260415 ISS+FPFH+LG 매칭 시각화"
    )
    parser.add_argument("--experiment", type=str, default="iss_fpfh_v1_20260415",
    ...
    dataset = MitsubishiV1ISSFPFHDataset(split=args.split)
    print(
    ...
    for idx in indices:
        ...
        batch = v1_iss_fpfh_collate_fn([sample])
