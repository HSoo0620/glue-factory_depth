"""
공식 pretrained SP+LG로 resample 데이터 매칭 테스트 (학습 없이 평가만).

사용법:
    python test_resample_official.py
    python test_resample_official.py --image_size 2880 --num_samples 20
    python test_resample_official.py --indices 0 10 50 90 --image_size 2880
"""
...
def visualize_pair(pred, data, idx, output_path, csv_path=None, gt_radius=3,
    ...
    """4색 시각화 (GT 좌표를 image_size 기준으로 스케일링)"""
...
def main():
    parser = argparse.ArgumentParser(description="Official SP+LG resample 평가")
    parser.add_argument("--device", type=str, default="cuda")
    parser.add_argument("--output_dir", type=str, default="results/resample_official_sp_lg")
    ...
    dataset = MitsubishiResampleDataset(split=args.split, image_size=args.image_size)
    print(f"{args.split} dataset: {len(dataset)} pairs (image_size={args.image_size})")
