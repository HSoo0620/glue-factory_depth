#!/bin/bash
# ISS(detector) + C++ SHOT352(descriptor) + LG 학습 (resample_2 데이터)
# C++ SHOT352: voxelSize=1.0, normalRadius=5.0, shotRadius=10.0, 352D, L2-norm

GPU_ID=${1:-3}
EXPERIMENT=${2:-"0407_resample2_iss_shot352_lg"}
IMAGE_SIZE=${3:-1751}
GT_RADIUS=${4:-20}
BATCH_SIZE=${5:-16}
RESTORE=${6:-""}
...
if [ "$NPZ_COUNT" -lt "$TOTAL_IMAGES" ]; then
    echo "=== ISS+SHOT352 cache incomplete (${NPZ_COUNT}/${TOTAL_IMAGES}). Precomputing... ==="
    python3 precompute_iss_shot352_resample2.py --image_size "$IMAGE_SIZE"
else
...
echo "=== Training ISS(det)+SHOT352(desc)+LG on resample_2 ==="
