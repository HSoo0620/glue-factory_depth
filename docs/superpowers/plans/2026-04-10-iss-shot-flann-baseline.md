# ISS+SHOT FLANN Baseline Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** LightGlue 없이 ISS+SHOT cached descriptors를 OpenCV FLANN으로 매칭하여 Registration baseline 성능을 평가하는 독립 스크립트 작성.

**Architecture:** `eval_registration_rule_based.py`의 구조를 따르되, detector+descriptor 부분을 캐시 로드+FLANN으로 교체. Core math 함수(pixel_to_grid3d, ransac_rigid 등)는 동일하게 복제. 양방향 RMSE 평가 파이프라인 재사용.

**Tech Stack:** Python, NumPy, OpenCV (FlannBasedMatcher), pandas, matplotlib
...
| 파일 | 역할 |
|---|---|
| Create: `eval_registration_iss_shot_flann.py` | 메인 평가 스크립트 |
| Reference: `eval_registration_rule_based.py` | 구조 참조 (core math, eval pipeline, visualization) |
| Reference: `eval_registration_iss_shot.py` | `kpts_to_orig` 좌표 변환 참조 |
| Data: `gluefactory/datasets/mitsubishi/iss_shot352_resample2_cache/*.npz` | 캐시된 ISS keypoints + SHOT descriptors |
| Data: `gluefactory/datasets/mitsubishi/outputs_txt/combination.csv` | Pair 목록 |

---

### Task 1: Core math 함수 + 상수 정의

**Files:**
- Create: `eval_registration_iss_shot_flann.py`

- [ ] **Step 1: 스크립트 생성 — imports, 상수, core math 함수**

`eval_registration_rule_based.py`에서 복제. 이 단계에서는 core math만 작성.

```python
"""
ISS+SHOT FLANN Baseline Registration 평가.

학습 기반 matcher 없이, 캐시된 ISS keypoints + SHOT descriptors를
OpenCV FLANN(KDTree)으로 매칭하여 Registration baseline 성능을 평가한다.
eval_registration_iss_shot.py (LightGlue)와 동일 RMSE 파이프라인으로 공정 비교.

좌표계: (u*grid_dx, v*grid_dy, depth_real) — RANSAC, R,t 추정, RMSE 모두 동일 공간

사용법:
    python eval_registration_iss_shot_flann.py --indices 0 10 50 90
    python eval_registration_iss_shot_flann.py --ratio_th 0.8 --num_samples 20
"""
...
# --- Core math (eval_registration_rule_based.py 동일) ---

def pixel_to_grid3d(keypoints_2d, depth_map_raw):
    """2D keypoint + raw depth -> Grid 3D (u*dx, v*dy, depth_real).
...
Run: `python -c "import ast; ast.parse(open('eval_registration_iss_shot_flann.py').read()); print('syntax OK')"`
Expected: `syntax OK`

- [ ] **Step 3: Commit**

```bash
git add eval_registration_iss_shot_flann.py
git commit -m "feat: add ISS+SHOT FLANN baseline script skeleton with core math"
```

---

### Task 2: 캐시 로드 + FLANN 매칭 + 좌표 변환 함수

**Files:**
- Modify: `eval_registration_iss_shot_flann.py`

- [ ] **Step 1: 캐시 로드, FLANN 매칭, 좌표 변환 함수 추가**

core math 함수 뒤, `# --- Cache & FLANN matching ---` 섹션으로 추가:

```python
# --- Cache loading & FLANN matching ---

def load_cache(cache_dir, depth_fname):
    ...
    kp = data["keypoints"][:n_valid]           # (n_valid, 2) crop 1751px 기준
    desc = data["shot_descriptors"][:n_valid]   # (n_valid, 352)
    return kp, desc
...
python -c "
from eval_registration_iss_shot_flann import load_cache, flann_match
from pathlib import Path
cache_dir = Path('gluefactory/datasets/mitsubishi/iss_shot352_resample2_cache')
kp0, desc0 = load_cache(cache_dir, 'depth_raw_0000.png')
kp1, desc1 = load_cache(cache_dir, 'depth_raw_0001.png')
print(f'kp0: {kp0.shape}, desc0: {desc0.shape}')
print(f'kp1: {kp1.shape}, desc1: {desc1.shape}')
idx0, idx1 = flann_match(desc0, desc1, ratio_th=0.75)
print(f'FLANN matches (ratio=0.75): {len(idx0)}')
idx0b, idx1b = flann_match(desc0, desc1, ratio_th=0.9)
print(f'FLANN matches (ratio=0.9): {len(idx0b)}')
"
```
Expected: keypoint shape 출력, FLANN 매칭 수 출력 (ratio 0.9 > 0.75)

- [ ] **Step 3: Commit**

```bash
git add eval_registration_iss_shot_flann.py
git commit -m "feat: add cache loading, FLANN matching, coordinate conversion"
```

---

### Task 3: 평가 파이프라인 (evaluate_one_direction + evaluate_pair)

**Files:**
- Modify: `eval_registration_iss_shot_flann.py`

- [ ] **Step 1: evaluate_one_direction 함수 추가**

`eval_registration_rule_based.py`와 동일. 좌표 변환 후 섹션에 추가:
...
- [ ] **Step 2: evaluate_pair 함수 추가**

캐시 로드 + FLANN 매칭 + 양방향 평가:
...
def evaluate_pair(cache_dir, master_fname, input_fname,
    ...
    # 캐시 로드
    kp0, desc0 = load_cache(cache_dir, master_fname)
    kp1, desc1 = load_cache(cache_dir, input_fname)
...
```bash
git add eval_registration_iss_shot_flann.py
git commit -m "feat: add evaluate_one_direction and evaluate_pair for FLANN baseline"
```

---

### Task 4: Visualization + Main 함수

**Files:**
- Modify: `eval_registration_iss_shot_flann.py`

- [ ] **Step 1: Visualization 함수 추가**

`eval_registration_rule_based.py`의 `warp_depth_to_master`와 `visualize_registration` 복제.
타이틀만 `[ISS+SHOT+FLANN]`으로 변경:
...
def visualize_registration(depth0_raw, depth1_raw, R_est, t_est,
    ...
    fig.suptitle(
        f"[ISS+SHOT+FLANN]  Matches: {n_matches}  |  3D: {n_3d}  |  "
...
def main():
    parser = argparse.ArgumentParser(
        description="ISS+SHOT FLANN Baseline Registration Eval")
    ...
    output_dir = Path(
        ...
        else f"results/registration/iss_shot_flann_eval")
    ...
    print(f"Method: ISS+SHOT+FLANN (cache: {args.cache_dir})")
    ...
    results = []
    for idx in tqdm(indices, desc="ISS+SHOT+FLANN"):
        print(f"\n--- Pair {idx} ---")
...
```bash
git add eval_registration_iss_shot_flann.py
git commit -m "feat: add visualization and main function for ISS+SHOT FLANN baseline"
```

---

### Task 5: End-to-end 실행 테스트

**Files:**
- Reference: `eval_registration_iss_shot_flann.py`
...
```bash
cd /home/jhs/work/Registration/glue-factory_depth
python eval_registration_iss_shot_flann.py --indices 0 --ratio_th 0.75
```

Expected output:
```
Method: ISS+SHOT+FLANN (cache: iss_shot352_resample2_cache)
...
--- Pair 0 ---
  Keypoints: view0=..., view1=...
  GT SVD: ... non-occluded pairs used
  Forward: matches=..., 3D=..., inliers=...
  ...
  Bidirectional RMSE: ...
```

성공 시 `results/registration/iss_shot_flann_eval/reg_test_00000.png`과 `rmse_test.csv` 생성 확인.
...
Expected: 4개 pair의 RMSE 결과 + Mean Bidirectional RMSE 출력.

- [ ] **Step 3: ratio_th 변화 확인**

Run:
```bash
python eval_registration_iss_shot_flann.py --indices 0 --ratio_th 0.9
```

Expected: ratio_th=0.75보다 매칭 수가 더 많아야 함 (ratio test가 더 관대).

- [ ] **Step 4: 결과 확인 후 Commit**

```bash
git add eval_registration_iss_shot_flann.py
git commit -m "test: verify ISS+SHOT FLANN baseline end-to-end"
```
