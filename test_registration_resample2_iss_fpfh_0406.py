"""
학습된 ISS(det)+FPFH(desc)+LG 모델로 resample_2 데이터 3D Registration 테스트.

resample_2는 물리적으로 보정된 이미지이므로 ini(카메라 파라미터) 없이
(u, v, depth_real)을 직접 3D 좌표로 사용.

파이프라인:
    1. 학습된 ISS+FPFH+LG로 crop+resize된 depth image pair 매칭
    2. 매칭된 keypoint를 원본 5761 좌표로 복원 → (u, v, depth_real)로 3D 좌표 생성
    3. RANSAC + SVD로 rigid transformation (R, t) 추정
    4. 변환 적용 후 정합 결과 시각화

사용법:
    python test_registration_resample2_iss_fpfh_0406.py --experiment 0402_resample2_iss_fpfh_xyz_lg --indices 0 10 50 90
    python test_registration_resample2_iss_fpfh_0406.py --checkpoint outputs/training/0402_resample2_iss_fpfh_xyz_lg/checkpoint_best.tar --conf_th 0.1 --inlier_th 5.0
"""
...
def visualize_registration(
    ...
    fig.suptitle(
        f"[Resample2 ISS+FPFH+LG]  Matches: {n_matches}  |  3D: {n_3d}  |  "
...
def main():
    parser = argparse.ArgumentParser(description="학습된 ISS+FPFH+LG 기반 resample_2 3D Registration")
    parser.add_argument("--checkpoint", type=str, default=None)
    parser.add_argument("--experiment", type=str, default="0402_resample2_iss_fpfh_xyz_lg")
