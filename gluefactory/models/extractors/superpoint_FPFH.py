...
class SuperPointFPFH(nn.Module):

    def __init__(self, conf):
        super().__init__()

        self.sp = SuperPoint(conf.superpoint)
        self.proj = nn.Linear(33,256)

    def depth_to_pointcloud(self, depth, keypoints):
    ...
    def forward(self, data):
        ...
        for b in range(B):
            ...
            dmap = depth[b,0].cpu().numpy()

            points = self.depth_to_pointcloud(dmap, kpts)

            fpfh = self.compute_fpfh(points)

            fpfh = torch.tensor(fpfh).float()

            descriptors.append(self.proj(fpfh))
