"""
v1 (2026-04-15) ISS + FPFH/SHOT + LightGlue 통합 학습 스크립트.
data.descriptor_kind ("fpfh" | "shot") 로 두 variant 중 선택.
"""
...
def training(rank, conf, output_dir, args):
    ...
    # v1 descriptor_kind dispatch (fpfh or shot) — 누락 시 KeyError (silent wrong 방지)
    descriptor_kind = conf.data.descriptor_kind
    if descriptor_kind == "fpfh":
        from .datasets.mitsubishi_v1_20260415_iss_fpfh_dataset import (
            MitsubishiV1ISSFPFHDataset as DSCls,
            v1_iss_fpfh_collate_fn as collate_fn_v1,
        )
    elif descriptor_kind == "shot":
        from .datasets.mitsubishi_v1_20260415_iss_shot_dataset import (
            MitsubishiV1ISSSHOTDataset as DSCls,
            v1_iss_shot_collate_fn as collate_fn_v1,
        )
    else:
    ...
    ds_kwargs = {}
    if "data_root" in conf.data:
        ds_kwargs["data_root"] = conf.data.data_root
    if "cache_dir" in conf.data:
        ds_kwargs["cache_dir"] = conf.data.cache_dir
    if "split_json" in conf.data:
        ds_kwargs["split_json"] = conf.data.split_json

    dataset = DSCls(split="train", **ds_kwargs)
    val_dataset = DSCls(split="val", **ds_kwargs)
