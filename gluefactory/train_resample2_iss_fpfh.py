...
def training(rank, conf, output_dir, args):
    ...
    fpfh_radius = conf.data.get("fpfh_radius", 5.0)
    coord_space = conf.data.get("coord_space", "xyz")
    batch_size = conf.data.get("batch_size", 4)
    num_workers = conf.data.get("num_workers", 4)

    dataset = MitsubishiResample2ISSFPFHDataset(split="train", fpfh_radius=fpfh_radius, coord_space=coord_space, image_size=image_size)
    val_dataset = MitsubishiResample2ISSFPFHDataset(split="val", fpfh_radius=fpfh_radius, coord_space=coord_space, image_size=image_size)
