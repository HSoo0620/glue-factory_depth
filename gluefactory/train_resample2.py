"""
Resample_2 데이터용 학습 스크립트.
고정 crop (3502x3502) + 선택적 리사이즈.

Author: Based on train_resample.py
"""
...
from .datasets.mitsubishi_resample2_dataset import MitsubishiResample2Dataset
from .datasets.mitsubishi_resample2_dataset import resample2_collate_fn
...
def training(rank, conf, output_dir, args):
    ...
    # image_size=None이면 crop 크기(3502) 그대로 사용
    image_size = conf.data.get("image_size", None)
    batch_size = conf.data.get("batch_size", 4)
    num_workers = conf.data.get("num_workers", 4)

    dataset = MitsubishiResample2Dataset(split="train", image_size=image_size)
    val_dataset = MitsubishiResample2Dataset(split="val", image_size=image_size)
    ...
    train_loader = DataLoader(
        ...
        collate_fn=resample2_collate_fn,
    )
