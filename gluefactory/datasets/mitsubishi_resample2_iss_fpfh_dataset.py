...
class MitsubishiResample2ISSFPFHDataset(Dataset):
    ...
    def __init__(self, split="train", cache_dir=None, fpfh_radius=5.0,
                 coord_space="xyz", image_size=None, base_dir=None):
        ...
        if cache_dir:
            ...
            self.cache_dir = self.base_dir / f"iss_fpfh_resample2_cache_r{fpfh_radius}_{coord_space}"

        if not self.cache_dir.exists():
            raise FileNotFoundError(
                f"ISS+FPFH cache not found: {self.cache_dir}\n"
                f"Run: python precompute_iss_fpfh_resample2{'_grid' if coord_space == 'grid' else ''}.py --fpfh_radius {fpfh_radius}"
            )
