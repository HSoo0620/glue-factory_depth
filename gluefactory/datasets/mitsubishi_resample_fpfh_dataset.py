"""
Resample 이미지 기반 FPFH 캐시 데이터셋.
precompute_fpfh_resample.py로 캐시 생성 후 사용.
"""
...
class MitsubishiResampleFPFHDataset(Dataset):
    ...
    def __init__(self, split="train", cache_dir=None, fpfh_radius=1.5,
        ...
        if cache_dir:
            ...
            self.cache_dir = self.base_dir / f"fpfh_resample_cache_r{fpfh_radius}"

        if not self.cache_dir.exists():
            raise FileNotFoundError(
                f"FPFH cache not found: {self.cache_dir}\n"
                f"Run: python precompute_fpfh_resample.py --fpfh_radius {fpfh_radius}"
            )
    ...
    def _load_fpfh_cache(self, img_rel_path):
        ...
        return {
            "keypoints": data["keypoints"],             # (N, 2) - image_size 기준 좌표
    ...
    def __getitem__(self, idx):
        ...
        # FPFH 캐시 로드 (keypoints는 이미 image_size 기준)
...
def resample_fpfh_collate_fn(batch):
