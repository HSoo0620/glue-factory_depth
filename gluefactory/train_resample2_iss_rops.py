"""
Resample_2 ISS+RoPS135 데이터용 학습 스크립트.
ISS(detector) + C++ RoPS135(descriptor) + LightGlue.
"""
...
from .datasets.mitsubishi_resample2_iss_rops_dataset import MitsubishiResample2ISSRoPSDataset
from .datasets.mitsubishi_resample2_iss_rops_dataset import resample2_iss_rops_collate_fn
...
def training(rank, conf, output_dir, args):
    ...
    image_size  = conf.data.get("image_size", None)
    cache_dir   = conf.data.get("cache_dir", None)
    batch_size  = conf.data.get("batch_size", 4)
    num_workers = conf.data.get("num_workers", 4)

    dataset     = MitsubishiResample2ISSRoPSDataset(split="train", cache_dir=cache_dir, image_size=image_size)
    val_dataset = MitsubishiResample2ISSRoPSDataset(split="val",   cache_dir=cache_dir, image_size=image_size)
    ...
    train_loader = DataLoader(
        ...
        collate_fn=resample2_iss_rops_collate_fn,
        pin_memory=True,
    )
    ...
    while epoch < conf.train.epochs and not stop:
                ...
                if conf.train.get("clip_grad", None):
                    ...
                    try:
                        torch.nn.utils.clip_grad_norm_(
                            all_params, max_norm=conf.train.clip_grad,
            ...
            if (
                ...
                with fork_rng(seed=conf.train.seed):
                    results, pr_metrics, figures = do_evaluation(
                        model, val_loader, device, loss_fn, conf.train, rank,
                        pbar=(rank == 0),
                    )

                if rank == 0:
                    _val_key = ["match_recall", "match_precision", "accuracy",
                                "loss/total", "loss/nll_pos", "loss/nll_neg"]
                    _val_parts = [f"{k}={results[k]:.4f}" for k in _val_key
                                  if k in results and isinstance(results[k], float)]
                    _val_rest  = [f"{k}={v:.4f}" for k, v in results.items()
                                  if isinstance(v, float) and k not in _val_key]
                    ...
                    if is_best:
                        ...
                        save_experiment(
                            ...
                            args.distributed, cp_name=cp_name,
                        )
                        shutil.copy(
            ...
            if (tot_it % conf.train.save_every_iter == 0 and tot_it > 0) and rank == 0:
                if results is None:
                    results, _, _ = do_evaluation(
                        model, val_loader, device, loss_fn, conf.train, rank, pbar=(rank == 0),
                    )
                    best_eval = results[conf.train.best_key]
                best_eval = save_experiment(
                    model, optimizer, lr_scheduler, conf, results,
                    best_eval, epoch, tot_it, output_dir, stop, args.distributed,
                )
            if stop:
                break
        ...
        if rank == 0:
            best_eval = save_experiment(
                model, optimizer, lr_scheduler, conf, results,
                best_eval, epoch, tot_it, output_dir=output_dir,
                stop=stop, distributed=args.distributed,
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--mixed_precision", "--mp", default=None, type=str,
                        choices=["float16", "bfloat16"])
    parser.add_argument("--compile", default=None, type=str,
                        choices=["default", "reduce-overhead", "max-autotune"])
    parser.add_argument("--cleanup_interval", default=120, type=int)
